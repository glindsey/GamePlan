...
bool StateMachine::add_state(State* state)
{
  ...
  {
    MAJOR_ERROR(boost::format("Attempted to add state \"%s\" to state machine \"%s\" more than once")
                % state_name.c_str()
                % this->get_name().c_str());
    return false;
  }
...
bool StateMachine::delete_state(std::string const& state_name)
{
  ...
  {
    MAJOR_ERROR(boost::format("Attempted to delete state \"%s\" while it was the current state")
                % state_name.c_str());
  }

  if (impl->state_map.count(state_name) == 0)
  {
    MAJOR_ERROR(boost::format("Attempted to delete state \"%s\" to state machine \"%s\" when it wasn't in there")
                % state_name.c_str()
                % this->get_name().c_str());
    return false;
  }
...
void StateMachine::render(sf::RenderTarget& target, int frame)
{
  if (impl->current_state != nullptr)
  {
    impl->current_state->render(target, frame);
  }
}
...
bool StateMachine::change_to(State* state)
{
  ...
  if (impl->current_state != nullptr)
  {
    ...
    {
      MINOR_ERROR(boost::format("Terminator for state \"%s\" in state machine \"%s\" returned false")
  ...
  if (terminator_result == true)
  {
    ...
    if (state != nullptr)
    {
      ...
      {
        MINOR_ERROR(boost::format("Initializer for state \"%s\" in state machine \"%s\" returned false")
                    % impl->current_state->get_name().c_str()
                    % this->get_name().c_str());
      }
    }
...
bool StateMachine::change_to(std::string const& new_state_name)
{
  ...
  {
    MAJOR_ERROR(boost::format("Attempted to change to state \"%s\" to state machine \"%s\" when it wasn't in there")
                % new_state_name.c_str()
                % this->get_name().c_str());
    return false;
  }
  else
